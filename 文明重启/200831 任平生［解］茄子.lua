...
function HLPZ(Search, Write, Type) gg.clearResults() gg.setVisible(false) HLPZ3(Search[1][1], Type) local count = gg.getResultCount() local result = gg.getResults(count) gg.clearResults() local data = {} local base = Search[1][2] if (count > 0) then for i, v in ipairs(result) do v.isUseful = true end
    ...
    if (#data > 0) then gg.toast("茄子帮你处理"..#data.."2条代码") local t = {} local base = Search[1][2] for i=1, #data do for k, w in ipairs(Write) do offset = w[2] - base t[#t+1] = {} t[#t].address = data[i] + offset t[#t].flags = Type t[#t].value = w[1] if (w[3] == true) then local item = {} item[#item+1] = t[#t] item[#item].freeze = true gg.addListItems(item) end end end
...
HLPZ3=gg.searchNumber
    ...
    function yiyz_yiyz_Main()
      menu = gg.choice({
        ...
        "加群反馈",
        "退出茄子"
      }, nil, "茄子科技/SVIP版〖自瞄无后已更新☆自瞄部分手机可用正在修复☆无后请用打架专区的全抢无后～有个弊端就是开镜一个点不开镜不会这个正在抓聚点先放出来给你们玩〗\n离开茄子时间:")
      if menu == 1 then
        A()
      end
      if menu == 2 then
...
function tsqb()
    F = gg.alert("茄子科技", "木墙透视","石墙透视","铁墙透视")
    if F == 1 then
