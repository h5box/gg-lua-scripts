...
function xgxc(A0_12, A1_13)
  ...
  L2_14 = 1
  L3_15 = #A1_13
  for _FORV_5_ = 1, #A1_13 do
    xgpy = A0_12 + A1_13[_FORV_5_].offset
    xglx = A1_13[_FORV_5_].type
    xgsz = A1_13[_FORV_5_].value
    gg.setValues({
      [1] = {
        address = xgpy,
        flags = xglx,
  ...
  if gg.getResultCount() == 0 then
    ...
    if gg.getResultCount() == 0 then
      ...
      for _FORV_4_ = 1, sz do
        pdsz = true
        for _FORV_8_ = 4, #A0_18 do
          if pdsz == true then
            pysz = {}
            pysz[1] = {}
            pysz[1].address = sl[_FORV_4_].address + A0_18[_FORV_8_].offset
            pysz[1].flags = A0_18[_FORV_8_].type
            szpy = gg.getValues(pysz)
            pdpd = A0_18[_FORV_8_].lv .. ";" .. szpy[1].value
            szpd = split(pdpd, ";")
            tzszpd = szpd[1]
            pyszpd = szpd[2]
      ...
      if xgjg == true then
        gg.toast(A0_18[2].name .. "开启成功,共修改" .. xgsl .. "条数据")
      else
        gg.toast(A0_18[2].name .. "开启失败")
        print(A0_18[2].name .. "开启失败")
      end
    end
  end
end
