...
function HLPZ(Search, Write, Type) gg.clearResults() gg.setVisible(false) HLPZ3(Search[1][1], Type) local count = gg.getResultCount() local result = gg.getResults(count) gg.clearResults() local data = {} local base = Search[1][2] if (count > 0) then for i, v in ipairs(result) do v.isUseful = true end
    ...
    if (#data > 0) then gg.toast("神仙帮你处理"..#data.."2条代码") local t = {} local base = Search[1][2] for i=1, #data do for k, w in ipairs(Write) do offset = w[2] - base t[#t+1] = {} t[#t].address = data[i] + offset t[#t].flags = Type t[#t].value = w[1] if (w[3] == true) then local item = {} item[#item+1] = t[#t] item[#item].freeze = true gg.addListItems(item) end end end
...
HLPZ3=gg.searchNumber
    ...
    gg.alert("神仙宫 科技群948360953")
    gg.toast("欢迎使用神仙宫科技 群948360953")
    function yiyz_yiyz_yiyz_Main()
      menu = gg.choice({
        "打架专区",
        "无后专区",
        "人物专区",
    ...
    function C()
      menu3 = gg.multiChoice({
        ...
        "倒地加速[稳定]",
        "请求返回主页面"
      }, nil, "如果靠近敌人天线消失请刷新人物坐标️/神仙宫科技 群948360953")
      if menu3 == nil then
      else
    ...
    function D()
      menu4 = gg.multiChoice({
        ...
        "透视墙壁",
        "请求返回主页面"
      }, nil, "人物穿墙前请先在大厅初始化，再进入游戏里面开启实体穿墙，携带两个油炸即可完成爆破/神仙宫科技 群948360953")
      if menu4 == nil then
      else
...
function tsqb()
    F = gg.alert("神仙宫科技", "木墙透视","石墙透视","铁墙透视")
    if F == 1 then
