...
function unXXX(str64)
 ...
 if lqs == nil then
   lqsa=gg.alert('请选择激活卡密,购卡联系2848823170','激活卡密','退出')
   if lqsa ~= 1 then print('激活失败') os.exit()
   else
     lqv = gg.prompt({'请输入您的激活码:','请将以下ID发送给Rick\nQQ.1262292336\n↓↓↓您的专属ID↓↓↓: ' .. lqdm},{[1]='',[2]= lqdm },{[1] = 'text',[2] = 'text'})
     ...
     if lqA == aaa then
     aaaa = 1
     gg.alert('激活码情况: 恭喜激活天卡\n价值:8','确定')
     else
       if lqA == bbb then
       aaaa = 7
       gg.alert('激活码情况: 恭喜激活周卡\n价值:45','确定')
       else
         if lqA == ccc then
         aaaa = 30
         gg.alert('激活码情况: 恭喜激活月卡\n价值:88','确定')
         else
         if lqA == ddd then
         aaaa = 5000
         gg.alert('激活码情况: 恭喜激活永久卡💫\n价值:💹无法估量','确定')
         else
           gg.alert('激活码错误 请加2848823170购买激活码') os.exit()
         end
       end
     end
   end
...
outb=info[2]..name.."主发卡机💞.lua"
outc=info[2]..name.."配置！"
warn = gg.alert('注入卡密系统完成', '确定', '退出')
