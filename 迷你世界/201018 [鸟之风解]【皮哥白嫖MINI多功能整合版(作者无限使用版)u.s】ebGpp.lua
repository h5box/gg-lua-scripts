---鸟之风解
---yic不有脚就行？
RX = gg.alert('⚠提示⚠：♦因您是作者您可以体验此脚本10000次,体验次数用完无法再次使用♦!','🔰点击进入🔰')
--这里填写脚本显示的试用次数相关提示
...
if FX2 == nil then
  mm = gg.prompt({[1]="⚠️️请输入一重脚本密码⚠️"}, {[1]="0"}, {[1]="txet"})
  ...
  mm = gg.prompt({[1]="⚠️️请输入双重脚本密码⚠️"}, {[1]="0"}, {[1]="txet"})
  if mm[1] == "4560"then
  else
    print(mm[1],"❌还不快求皮哥给你脚本密码❌  ❗❗❗❗❗❗❗❗❗❗❗")
    os.exit()
  end
  mm = gg.prompt({[1]="⚠️️请输入三重脚本密码⚠️"}, {[1]="0"}, {[1]="txet"})
  if mm[1] == "12745" then
    gg.toast("🎉开启四重密码🔒🎉")
  else
  ...
  mm = gg.prompt({[1]="⚠️️请输入四重脚本密码⚠️"}, {[1]="0"}, {[1]="txet"})
  if mm[1] == "1238" then
    gg.toast("💯")
  else
    print(mm[1],"🎌还不快求皮哥给你脚本密码🎌  ⭕❌⭕❌⭕❌⭕❌⭕❌⭕❌⭕❌⭕")
...
gg.alert('⚽️皮哥留言⚽️')
gg.alert('⚽关注我了么？⚽')
gg.alert('🏆快去关注我 手机号17809248677在快手搜🏆')
gg.alert('🏆关注了？🏆')
gg.alert('🏀谢谢你的支持🏀')
gg.alert('🏀(=^▽^=)🏀')
gg.alert('🏁关注了不要取关🏁')
