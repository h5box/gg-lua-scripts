---鸟之风解
---yic不有脚就行？
RX = gg.alert('⚠提示⚠：♦因您是作者您可以体验此无限脚本次,♦!','🔰点击进入🔰')
--这里填写脚本显示的试用次数相关提示
 if rx == 1 then os.exit() end 
 if rx == 2 then os.exit() end 
 file=io.open('/storage/emulated/0/tencent/QQfile_recv/.thumbnails/这里填写你要运行的脚本名字','a+') 
 file:write('2') 
 cs = file:seek('end')  
 ss = 10000000000000000000000000--这里填写脚本试用的次数
 if cs < ss then  
 gg.alert('⚠提示⚠：🔰您当前使用次数为'..cs..'次!,剩余可用次数为'..ss-cs..'次!🔰') 
 else  
 gg.alert('🔰提示🔰：当前脚本使用次数已用完🔰!') 
 os.exit() 
 end  
 tg = gg.alert('💎提示：验证已通过,是否进入?💎','💊点击进入💊') 
 if rx1 == 2 then end
 if rx1 == 2 then end
 if rx1 == 3 then os.exit() end
 
 


if FX2 == nil then
  mm = gg.prompt({[1]="⚠️️请输入一重脚本密码（密码6957）⚠️"}, {[1]="0"}, {[1]="txet"})
  ...
  "6957"
   ...
   then
    ...
    print(mm[1],"♦还不快求皮哥给你脚本密码♦  🔰🔰🔰🔰🔰🔰🔰🔰💢💢")
    os.exit()
  end
  mm = gg.prompt({[1]="⚠️️请输入双重脚本密码（密码4560）⚠️"}, {[1]="0"}, {[1]="txet"})
  if mm[1] == "4560"then
  else
    print(mm[1],"❌还不快求皮哥给你脚本密码❌  ❗❗❗❗❗❗❗❗❗❗❗")
    os.exit()
  end
  mm = gg.prompt({[1]="⚠️️请输入三重脚本密码（密码12745）⚠️"}, {[1]="0"}, {[1]="txet"})
  if mm[1] == "12745" then
    gg.toast("🎉开启四重密码🔒🎉")
  else
    print(mm[1],"〽还不快求求皮哥给你脚本密码〽  ⭕❌⭕❌⭕❌⭕❌⭕❌⭕❌⭕❌⭕")
    os.exit()
  end
  mm = gg.prompt({[1]="⚠️️请输入四重脚本密码（密码1238）⚠️"}, {[1]="0"}, {[1]="txet"})
  if mm[1] == "1238" then
    gg.toast("💯")
  else
    print(mm[1],"🎌还不快求皮哥给你脚本密码🎌  ⭕❌⭕❌⭕❌⭕❌⭕❌⭕❌⭕❌⭕")
...
SN = gg.choice({
	 ...
	 "⬜🔒 未 添 加 🔒⬜",
	 "❌❌❌请自觉中断   🏁ELXT🏁   脚本退出❌❌❌"
}, nil, "🎱作者我爱:迷你号1055178806\n🎯{🎬🎬🎬🎬}\n我爱破解♦❌⭕⭕❌♦\n🔰💍⚽❤🎀感谢您的使用🎀❤💍⚽🔰\n💎该版本等待更新💎=🔱皮哥2.1出版🔱｛开启时间"..SJ.."｝")
if SN==1 then
	 Main1()
end
if SN==2 then
