 ...
 * 程序说明
 * @package   FileBox
 * @author    制作by<jooies@ya.ru> 优化by<413316602@qq.com>
...
$udir = 'admin_box';
$ver = '1.10.2';
$safe_num = 10; //设置多少次后禁止登陆，为0则不限制，建议为3-5
$mail = '413316602@qq.com'; //若有恶意登录，会发邮件到这个邮箱，前提是mail()函数可用！
...
function maintop($title,$showtop = true) {
    ...
    if ($showtop) {//头部菜单内容
      if($op=='up'||$op=='upload'||$op=='yupload')$up = "class='curr'";
	  if($op=='home'||$op =='edit'||$op =='ren'||$op =='unz')$home = "class='curr'";
	  if($op=='cr'||$op=='create')$cr = "class='curr'";
	  if($op=='sqlb'||$op=='sqlbackup')$sqlb = "class='curr'";
	  if($op=='ftpa'||$op=='ftpall')$ftpa = "class='curr'";
	  if($op=='tz'||$op=='tanzhen')$tz = "class='curr'";
        echo "<div class='menu'>\n<ul><a href='{$meurl}?op=home'><li $home>主页</li></a>\n"
			...
			."<a href='{$meurl}?op=tz'><li $tz>PHP探针</li></a>\n"
...
function tz(){
	global $udir;
	maintop("PHP探针");
	echo "<div class='title'>php自定义探针 <div class='right'><a href='$udir/u.php'>高级</a></div></div><div class='box'>服务端IP:{$_SERVER['SERVER_ADDR']} <br>客户端IP:".get_ip()."<br />user-agent：".$_SERVER['HTTP_USER_AGENT']."</div>";
	echo '';
	mainbottom();
}
...
function get_ip($type=0,$client=true)
{
	$type=$type?1:0;
	static $ip=NULL;
